...
class SMArT_GPT2(nn.Module):
    def __init__(self, configs, device):
        super(SMArT_GPT2, self).__init__()
        ...
        if configs.is_gpt:
            if configs.pretrain:
                self.gpt2 = GPT2Model.from_pretrained('../pretrain/GPT2',
        ...
        self.attn_layer = MultiLayerAttention(self.patch_size, self.num_embeddings, num_layers=4, num_heads=4, dropout=0.3)
        ...
        self.text_prompt = torch.load('../embeddings/text_prompts_GPT2.pt')
        ...
        for layer in (self.gpt2, self.num_layer, self.patch_layer, self.attn_layer, self.image_layer, self.fusion_layer,
    ...
    def forward(self, x):
        ...
        # Reasoning
        emd = self.gpt2(inputs_embeds=emd).last_hidden_state[:, -self.patch_num:, :]
